...
# describe how is gonna extract the inside of each document
LOADER_MAPPING = {
...
#we load the inside of each document

def load_single_document(file_path: str) -> (List[Document]):
    ext = file_path.rsplit(".", 1)[-1]
    if ext in LOADER_MAPPING:
        loader_class, loader_args = LOADER_MAPPING[ext]
        loader = loader_class(file_path, **loader_args) # the list of objects
        content = loader.load()
        return content
